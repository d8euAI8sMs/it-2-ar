...
namespace plot
{
	...
	tick_factory::formatter_t make_formatter(int precision)
	{
	...
	public:
		...
		virtual void draw(CDC &dc, const viewport &bounds) override
		{
            ...
            //str = _T("X");
            //tbounds = dc.GetTextExtent(str);
            //dc.TextOut(new_bounds.screen.xmax - tbounds.cx, bounds.screen.ymax - y_offset - 2, str);
            //str = _T("Y");
            //tbounds = dc.GetTextExtent(str);
            //dc.TextOut(x_offset - tbounds.cx - 2, new_bounds.screen.ymin, str);
	...
	public:
		...
		this_t with_data(std::shared_ptr<data_t> data, data_painter_t painter = list_drawable::circle_painter())
		{
			layers->with(new list_drawable(list_drawable::const_data_factory(data), std::move(painter)));
			return *this;
        }
